...
namespace CRIMAS.SupportClasses
{
    ...
    {
        public static string UploadImage(Stream inputStream, string fileName, BlobContainer blobContainer)
        {
            if (inputStream != null && inputStream.Length > 0)
            {
                ...
                dirPath = blobContainer.ToString().ToLowerInvariant();
                ...
                if (blobContainer == BlobContainer.customer)
                {
                    inputStream = ResizeImage(inputStream, 500);
                    inputStream.Seek(0, SeekOrigin.Begin);
                }

                inputStream.Seek(0, SeekOrigin.Begin);
                CloudBlockBlob thumbBlob = container.GetBlockBlobReference(fileName);
        ...
        public static void DeleteBlob(BlobContainer blobContainer, string filename)
        {
            if (!string.IsNullOrWhiteSpace(filename))
            {
                filename = filename.Replace(ConfigurationManager.AppSettings["Azure:StorageUrl"] + blobContainer.ToString() + "/", "");
                // Retrieve storage account from connection string.
                CloudStorageAccount storageAccount = CloudStorageAccount.Parse(ConfigurationManager.AppSettings["Azure:StorageConnectionString"]);

                // Create the blob client.
                CloudBlobClient blobClient = storageAccount.CreateCloudBlobClient();
                CloudBlobContainer container = blobClient.GetContainerReference(blobContainer.ToString().ToLowerInvariant());
                if (!container.Exists()) container.Create(BlobContainerPublicAccessType.Blob);
